...
# This has same result as did in lecture
# return: children -> list of list

def correction(child,search_list):
    appear_hash = {}
    search_point = 0
    for each in range(len(child)):
        if child[each] not in appear_hash:
            appear_hash[child[each]] = 1
        else:
            while search_point < len(search_list):
                if search_list[search_point] not in appear_hash:
                    child[each] = search_list[search_point]
                    appear_hash[child[each]] = 1
                    search_point += 1
                    break
                elif search_list[search_point] in appear_hash:
                    search_point += 1
                    continue
    return child
...
def crossover(parents):
    ...
    cut_point = random.choice(range(size))
    parent1 = parents[0]
    parent2 = parents[1]
    
    child1_new_part1 = parent1[:cut_point]
    child1_new_part2 = parent2[cut_point:]
    
    child2_new_part1 = parent2[:cut_point]
    child2_new_part2 = parent1[cut_point:]
    
    child1 = child1_new_part1 + child1_new_part2
    child2 = child2_new_part1 + child2_new_part2
    
    child1 = correction(child1,parent2)
    child2 = correction(child2,parent1)
...
def genetic(
    ...
    while end - start < 40:
        ...
        while len(all_children) < n_pop:
            children = mutate(parents,p_mut,population,n_cities)
...
tour,tour_length = genetic(p_mut=0.05,n_pop = 100,num_parent = 30)  
