...
# 2: Update every route by adding 1/distance
# return: pheromne: pheremone matrix -> list of list
def UpdatePheromne(pheromne,RHO,ant_route,n_cities,ant_cost_hash):
    for row in range(len(pheromne)):
        for column in range(len(pheromne)):
            pheromne[row][column] *= (1-RHO)
            if pheromne[row][column] < 10 ** -50:
                pheromne[row][column] = 10 ** -30
    
    for each_ant in range(len(ant_route)):
        each_ant_cost = ant_cost_hash[each_ant]
        
        for each_city in range(-1,n_cities-1):
            pheromne[ant_route[each_ant][each_city]][ant_route[each_ant][each_city+1]] += 1/each_ant_cost
...
# AOC algorithm:
# 1: Initializing the pheremone matrix
# 2: Get route for each ant
# 3: Get cost for each ant
# 4: Update pheremone matrix
# 5: Choose best route and cost
# 6: loop 2-5 until time stop
def AOC(
    alpha=1,
    beta=3,
    rho=0.8,
    ...
    real_best_cost = float('inf')
    real_best_route = []
    while time.time() - start_time < 40:
        ant_route = GetRoute(n_ants,alpha,beta,n_cities,pheremone_matrix)
        ant_cost_hash = GetAllCost(ant_route)
        pheremone_matrix = UpdatePheromne(pheremone_matrix,rho,ant_route,n_cities,ant_cost_hash)
        best_cost = min(ant_cost_hash.values())
        place_best_route = [key for key in ant_cost_hash if ant_cost_hash[key] == best_cost][0]
        best_route = ant_route[place_best_route]
        if real_best_cost > best_cost:
            real_best_cost = best_cost
            real_best_route = best_route
    return real_best_route,real_best_cost

tour,tour_length = AOC(alpha=1,beta=3,rho=0.8,n_ants=20,t=3)
